...
The markdown subset supported is the one the report uses: headings, paragraphs with ``**bold**``
inline, ``-`` bullets, pipe tables, ``![caption](path)`` figures and ``---`` page breaks. It is a
renderer for this document, not a general markdown engine.
...
# A4. Margins and type are sized to land the whole report in five pages; see TARGET_PAGES.
PAGE_W, PAGE_H = 8.27, 11.69
MARGIN_L, MARGIN_R = 0.66, 0.66
MARGIN_T, MARGIN_B = 0.55, 0.48
BODY_W = PAGE_W - MARGIN_L - MARGIN_R

# The body text is monochrome by choice: printed flat, and nothing in the argument depends on a
# colour being reproduced. The figures are in colour, where hue carries real information.
SAVE_DPI = 300       # resolution of embedded bitmaps only; all type is vector
FONT = "DejaVu Sans"
SZ_TITLE, SZ_H2, SZ_H3, SZ_BODY, SZ_CAPTION, SZ_TABLE = 14.0, 10.0, 9.0, 7.9, 7.0, 7.2
LEAD = 1.17          # line spacing, multiples of font size
INK = "#1a1a1a"
ACCENT = "#1a1a1a"   # headings are set in the body colour; weight carries the hierarchy
RULE = "#b4b4b4"
...
class Page:
    ...
    def close(self):
        self.fig.text(self.fx(PAGE_W / 2 - MARGIN_L), self.fy(PAGE_H - MARGIN_B + 0.34),
                      str(self.number), fontsize=7.5, color="#8a8a8a", family=FONT, ha="center")
...
def line_width(doc: Doc, toks: list[Tok], size: float) -> float:
    ...
    for i, tok in enumerate(toks):
        if i > 0 and not tok.glue:
            total += space_w
        total += doc.page.width_of(tok.text, _fontsize(tok, size), tok.weight, tok.family)
    return total
...
def line_count(doc: Doc, toks: list[Tok], width_in: float, size: float) -> int:
    ...
    n, cur = 1, 0.0
    for tok in toks:
        w = doc.page.width_of(tok.text, _fontsize(tok, size), tok.weight, tok.family)
...
def render_heading(doc: Doc, level: int, text: str) -> None:
    ...
    if level == 2:
        ...
        doc.page.fig.add_artist(plt.Line2D(
            [doc.page.fx(MARGIN_L), doc.page.fx(MARGIN_L + BODY_W)],
            [doc.page.fy(doc.page.y)] * 2, color=RULE, lw=0.6))
    doc.space(0.055)
...
def render_figure(doc: Doc, caption: str, rel_path: str) -> None:
    ...
    cap_h = 0.26 if cap_words else 0.0
    if not doc.page.room(h_in + cap_h + 0.18):
        doc.break_page()
