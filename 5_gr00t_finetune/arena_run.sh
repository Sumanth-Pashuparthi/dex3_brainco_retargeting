...
#
# Same wrapper as the earlier steps, with one extra mount: $CKPT_ROOT at /ckpt, so a policy config
# can point at a fine-tuned checkpoint without copying it into the models directory.
#
...
[ -d "$ARENA_DIR" ] || { echo "Arena checkout not found at $ARENA_DIR. Run 1_baseline/setup.sh." >&2; exit 1; }
mkdir -p "$EVAL_DIR" "$CKPT_ROOT"
